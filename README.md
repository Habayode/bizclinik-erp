...
A Nigerian SME ERP with a proper double-entry general ledger. SQLite backs
local/dev and single-tenant installs; Postgres backs multi-tenant production —
both share one SQLAlchemy model layer. Modules: Sales (quote → order → invoice →
