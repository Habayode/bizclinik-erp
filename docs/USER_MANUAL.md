...
5. **Suppliers** — S001 FreshFarm Produce, S002 PackRight Supplies, S003 Lagos Properties.
   ...
   > **Migrating existing data?** Most master-data screens have a **📥 Bulk import from Excel** option: download a template, fill one row per record, and upload to add them all at once instead of one by one. Available for **Customers, Suppliers & Bank accounts** (Settings), **Products** (Inventory), **Employees** (HR → Employees → Add), and the **Chart of Accounts** (General Ledger). Only *name* is required; *code* auto-generates if blank (except account codes, which you supply). Bank accounts auto-create a matching GL account if you don't name one. Re-uploading is safe — existing codes are skipped, and the Instructions sheet in each template explains every column.
   >
   > **Going live with real figures?** Use **General Ledger → 🏁 Opening balances** to upload your closing **trial balance** (one GL account per row, debit or credit). Trakit365 posts it as a single balanced opening journal as at your go-live date; a small out-of-balance difference can be absorbed into a balancing account (e.g. Retained Earnings / Opening Balance Equity). It runs once — void the opening journal to re-import.
